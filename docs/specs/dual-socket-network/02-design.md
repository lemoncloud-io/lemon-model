...
- **재번호 일관성**: index는 배열의 현재 위치가 유일한 진실이다. 제거로 배열이 당겨지면 행 라벨·재생성된 합성체의 `ctx.index`뿐 아니라 **살아남은 백업들의 직접 구독 태깅 클로저도 새 index로 재구독**해야 한다 — 한 소켓이 채널별로 다른 번호로 표기되는 갈라짐 방지. Remove 테스트는 수신 이벤트의 `socketIndex` 값까지 단언한다.
