...
- 워터마크 (watermark): 타입별로 pull을 통해 반영된 모델의 최대 updatedAt. 단조 증가하며 pull의 since 인자가 된다. 이벤트 반영은 워터마크를 움직이지 않는다.
...
- 동기화 머신은 CoreModel을 상속한 도메인 모델(예: UserModel)을 등록하는 것만으로 동작한다. id·updatedAt을 갖춘 모델이면 CoreModel 밖 대상도 등록할 수 있다.
